...
global{ 
	//Input the dataset.
	file road_shapefile <- file("C:/三明治/courses/sptial simulation/baseData/includes/my_major.shp");
	...
	reflex save_result when: every(50#cycles)
	{
		b <- b+50;
		save plot to:"C:/三明治/courses/sptial simulation/result/grid"+ b +".asc" format:asc;
	}
...
grid plot file: base_dem use_individual_shapes: false use_regular_agents: false neighbors: 4{
	...
	action compute_distances
	{
		...
		centers c <- centers closest_to self;
		dist_center <- (self distance_to c) using topology(world); //当前位置到中心点的距离
		...
		dist_level <- route_pp.level;	//求算最近道路的等级
	}
	action build
	{
		grid_value <- 1.0;
	...
	{ 
		...
		return (densite * w_neibor_dens + dist_route * w_road_dist + dist_cv * w_center_dist + w_slope*n_slope+w_road_level*dist_level) / (w_slope+w_neibor_dens + w_road_dist + w_center_dist);
	}	
}
experiment grow_01 type: gui {
	...
	parameter "Weight of road level" var: w_road_level;
 	...
 	output {
		...
		display buildingnum_chart background:#white{
		...
		display construct_chart background:#white{
